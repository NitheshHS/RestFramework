...
public class FileLib {
	public String getCommonKeyValue(String value)
	{
		...
		try {
			FileInputStream fi=new FileInputStream("./src\\commondata\\commondata.properties");
			prop=new Properties();
			prop.load(fi);	
		} catch (Exception e) {
			e.printStackTrace();
	...
	public String readDataInExcel(String sheetname, int rowNum, int cellnum) {
		Sheet sh = null;
		try {
			FileInputStream file =new FileInputStream("./src\\commondata\\TestLead.xlsx");
	...
	public Object[][] readAllDataFromExcel(String sheetName){
		...
		try {
			...
			FileInputStream fil=new FileInputStream("./src\\commondata\\ContactTest.xlsx");
